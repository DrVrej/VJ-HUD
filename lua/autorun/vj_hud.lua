	...
	net.Receive("vj_hud_ent_info", function(len, pl)
		...
		if IsValid(pl) && IsValid(ent) then
			...
			if ent:IsNPC() then
				...
				local npc_followingplyn = (ent.FollowingPlayer == true and ent.FollowPlayer_Entity:Nick()) or i18n.GetPhrase("unknow")
				if npc_followingplyn == pl:Nick() then npc_followingplyn = i18n.GetPhrase("you") end
...
hook.Add("HUDPaint", "vj_hud_crosshair", function()
	...
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_ch_enabled"):GetInt() == 0 then return end
...
hook.Add("HUDPaint", "vj_hud_ammo", function()
	local ply = LocalPlayer()
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_ammo"):GetInt() == 0 then return end
	if ply:InVehicle() && GetConVar("vj_hud_ammo_invehicle"):GetInt() == 0 then return end
	
	local curwep = ply:GetActiveWeapon()
	if not IsValid(curwep) or ply:GetActiveWeapon() == "Camera" then
		return -- Don't draw the ammo counter
	end
	
	-- Poon abranknere
	draw.RoundedBox(1, ScrW()-195, ScrH()-130, 180, 95, COLOR_000150)
	...
	if suit_power < 50 or flashlight == true then
		...
		draw.RoundedBox(8, ScrW()-260, ScrH()-75, 60, 40, COLOR_000150)
	end
	...
	draw.SimpleText(ply:GetAmmoCount("grenade"),"VJFont_Trebuchet24_MediumLarge", ScrW()-70, ScrH()-70, color_white, 0, 0)
	...
	if curwep:IsWeapon() then
		...
		draw.SimpleText(wepname,"VJFont_Trebuchet24_Small", ScrW()-185, ScrH()-125, color_white, 0, 0)
	end
	
	local hasammo = true
	local ammo_not_use = false -- Does it use ammo? = true for things like gravity gun or physgun
	local ammo_pri = pri_clip.." / "..pri_extra
	local ammo_pri_c = COLOR_02550150
	local ammo_sec = sec_ammo
	local ammo_sec_c = COLOR_000150
	...
	if pri_clip == -1 && curwep:GetSecondaryAmmoType() == -1 then -- Uses primary only with no ammo reserve, ex: "weapon_rpg" or "weapon_frag"
		ammo_pri = pri_extra
		ammo_pri_c = COLOR_02550150
		ammo_sec = "---"
		ammo_sec_c = COLOR_2551000150
	end
	...
	if curwep:GetPrimaryAmmoType() == -1 && curwep:GetSecondaryAmmoType() == -1 then -- Doesn't use ammo
		ammo_not_use = true
		ammo_pri = "---"
		ammo_pri_c = COLOR_2551000150
		ammo_sec = "---"
		ammo_sec_c = COLOR_2551000150
	elseif hasammo == false then -- Primary empty!
		ammo_pri = i18n.GetPhrase("empty")
		ammo_pri_c = color(255, 0, 0, empty_blink)
	end
	...
	if ammo_not_use == false then
		...
		if model_vm:GetSequenceActivity(model_vm:GetSequence()) == ACT_VM_RELOAD or string.match(model_vm:GetSequenceName(model_vm:GetSequence()), "reload") != nil then
			...
			if anim_perc < 100 then
				draw.RoundedBox(8, ScrW()-195, ScrH()-160, 180, 25, COLOR_025525540)
				draw.RoundedBox(8, ScrW()-195, ScrH()-160, math.Clamp(anim_perc, 0, 100)*1.8, 25, COLOR_0255255160)
				draw.RoundedBox(8, ScrW()-195, ScrH()-160, 180, 25, COLOR_000150)
				draw.SimpleText(anim_dur.."s ("..anim_perc.."%)", "VJFont_Trebuchet24_SmallMedium", ScrW()-137, ScrH()-156, color_white, 0, 0)
			end
		end
	end
end)
...
hook.Add("HUDPaint", "vj_hud_health", function()
	...
	if not ply:Alive() then -- Meradz tsootsage
		local deadhealth_blinka = math.abs(math.sin(CurTime() * 6) * 200)
		local deadhealth_blinkb = math.abs(math.sin(CurTime() * 6) * 255)
		if string.len(i18n.GetPhrase("user_dead")) > 9 then
			draw.RoundedBox(8, 70, ScrH()-80, 142 + (string.len(i18n.GetPhrase("user_dead")) * 5), 30, color(150, 0, 0, deadhealth_blinka))
		else
			draw.RoundedBox(8, 70, ScrH()-80, 142, 30, color(150, 0, 0, deadhealth_blinka))
		end

		draw.SimpleText(i18n.GetPhrase("user_dead"), "VJFont_Trebuchet24_Medium", 85, ScrH()-77, color(255, 255, 0, deadhealth_blinkb), 0, 0)
	else
		draw.RoundedBox(1, 15, ScrH()-130, 245, 95, COLOR_000150)
		local hp_r = 0
		local hp_g = 255
		local hp_b = 0
		...
		if ply:GetNW2Bool("vj_hud_godmode") == true then
			...
			hp_g = 102
			hp_b = 255
			draw.RoundedBox(8, 15, ScrH()-160, 155, 25, COLOR_000150)
			draw.SimpleText(i18n.GetPhrase("god_mod_enabled"), "VJFont_Trebuchet24_SmallMedium", 25, ScrH()-156, color(hp_r, hp_g, hp_b, 255), 0, 0)
		else
			local warning = 0
			if lerp_hp <= 35 then
			...
			if warning == 1 then
				if string.len(i18n.GetPhrase("low_health")) > 20 then
					draw.RoundedBox(8, 15, ScrH()-160, 180 + (string.len(i18n.GetPhrase("low_health")) * 2), 25, color(150, 0, 0, math.abs(math.sin(CurTime() * 4) * 200)))
				else
					draw.RoundedBox(8, 15, ScrH()-160, 180, 25, color(150, 0, 0, math.abs(math.sin(CurTime() * 4) * 200)))
				end

				draw.SimpleText(i18n.GetPhrase("low_health"), "VJFont_Trebuchet24_SmallMedium", 25, ScrH()-156, color(255, 153, 0, math.abs(math.sin(CurTime() * 4) * 255)), 0, 0)
			elseif warning == 2 then
				if string.len(i18n.GetPhrase("death_imminent")) > 20 then
					draw.RoundedBox(8, 15, ScrH()-160, 220 + (string.len(i18n.GetPhrase("death_imminent")) * 1.5), 25, color(150, 0, 0, math.abs(math.sin(CurTime() * 6) * 200)))
				else
					draw.RoundedBox(8, 15, ScrH()-160, 220, 25, color(150, 0, 0, math.abs(math.sin(CurTime() * 6) * 200)))
				end

				draw.SimpleText(i18n.GetPhrase("death_imminent"), "VJFont_Trebuchet24_SmallMedium", 25, ScrH()-156, color(255, 153, 0, math.abs(math.sin(CurTime() * 6) * 255)), 0, 0)
			end
		end
		...
		surface.SetDrawColor(COLOR_000150)
		surface.DrawTexturedRect(22, ScrH()-80, 40, 40)
		draw.SimpleText(string.format("%.0f", lerp_armor).."%", "VJFont_Trebuchet24_Medium", 70, ScrH()-83, COLOR_0255255160, 0, 0)
		draw.RoundedBox(0, 70, ScrH()-60, 180, 15, COLOR_025525540)
		draw.RoundedBox(0, 70, ScrH()-60, math.Clamp(lerp_armor, 0, 100)*1.8, 15, COLOR_0255255160)
		...
		if suit_power < 100 then
			...
			draw.SimpleText(i18n.GetPhrase("suit"), "VJFont_Trebuchet24_Tiny", 70, ScrH()-89, color(suit_r, suit_g, 0, 255), 0, 0)
		end
	end
end)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
hook.Add("HUDPaint", "vj_hud_localplayerinfo", function()
	local ply = LocalPlayer()
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_playerinfo"):GetInt() == 0 then return end
	draw.RoundedBox(1, 260, ScrH()-130, 200, 95, COLOR_000150)
	
	-- Number of kills
	surface.SetMaterial(mat_knife)
	surface.SetDrawColor(COLOR_255255255150)
	surface.DrawTexturedRect(260, ScrH()-125, 28, 28)
	draw.SimpleText(ply:Frags(), "VJFont_Trebuchet24_Medium", 293, ScrH()-125, COLOR_255255255150, 100, 100)
	
	-- Number of deaths
	surface.SetMaterial(mat_skull)
	surface.SetDrawColor(COLOR_255255255150)
	surface.DrawTexturedRect(260, ScrH()-95, 28, 28)
	draw.SimpleText(ply:Deaths(), "VJFont_Trebuchet24_Medium", 293, ScrH()-93, COLOR_255255255150, 100, 100)
	...
	draw.SimpleText(kd, "VJFont_Trebuchet24_Medium", 293, ScrH()-63, COLOR_255255255150, 100, 100)
	
	-- Movement speed
    local speed;
	if hud_unitsystem == 1 then
		speed = math.Round((ply:GetVelocity():Length() * 0.04263382283) * 1.6093).."KM/H"
	else
	...
	draw.SimpleText(speed, "VJFont_Trebuchet24_Medium", 373, ScrH()-125, COLOR_255255255150, 100, 100)
	...
	draw.SimpleText(fps.."fps", "VJFont_Trebuchet24_Medium", 373, ScrH()-93, COLOR_255255255150,0,0)
	...
	if IsValid(ply:GetVehicle()) then
		draw.RoundedBox(1, 320, ScrH()-160, 140, 30, COLOR_000150)
		...
		if hud_unitsystem == 1 then
			speedcalc = math.Round((speedcalc * 0.04263382283) * 1.6093).."KM/H"
		else
		...
		draw.SimpleText(speedcalc, "VJFont_Trebuchet24_Medium", 373, ScrH()-155, COLOR_255255255150, 100, 100)
	end
end)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------ Compass ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
hook.Add("HUDPaint", "vj_hud_compass", function()
	local ply = LocalPlayer()
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_compass"):GetInt() == 0 then return end
	draw.RoundedBox(1, ScrW() / 2.015, 10, 60, 60, COLOR_000150)
	local ang = ply:GetAngles().y
	local comp_dir = i18n.GetPhrase("unknow")
	...
	draw.SimpleText(comp_dir, "VJFont_Trebuchet24_Large", ScrW() / 1.955, 26, COLOR_0255255255, 1, 1)
	...
	draw.SimpleText(distrl, "VJFont_Trebuchet24_Small", ScrW() / (1.985 - move_ft), 40, COLOR_255255255200, 0, 0)
	...
	draw.SimpleText(dist.." WU", "VJFont_Trebuchet24_Tiny", ScrW()/(1.975-move_wu), 55, COLOR_0255255255, 0, 0)
end)
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------ Trace Information ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
hook.Add("HUDPaint", "vj_hud_traceinfo", function()
	local ply = LocalPlayer()
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_trace"):GetInt() == 0 then return end
	local trace = util.TraceLine(util.GetPlayerTrace(ply))
	if IsValid(trace.Entity) then
		local ent = trace.Entity
		if not ent:IsNPC() && not ent:IsPlayer() && GetConVar("vj_hud_trace_limited"):GetInt() == 1 then return end -- Yete limited option terver e, mi sharnager
		...
		if pos.visible then
			...
			net.WriteEntity(ent)
			net.SendToServer()
			draw.SimpleText(distrl.."("..dist.." WU)", "VJFont_Trebuchet24_SmallMedium", pos.x, pos.y - 26, COLOR_0255255255, 0, 0)
			draw.SimpleText(language.GetPhrase(ent:GetClass()), "VJFont_Trebuchet24_Medium", pos.x, pos.y - 12, color_white, 0, 0)
			draw.SimpleText(tostring(ent), "VJFont_Trebuchet24_Small", pos.x, pos.y + 10, COLOR_255255255200, 0, 0)
			...
			if ent:IsNPC() then -- NPC-ineroon hamar minag:
				...
				if string.sub(npc_info, 1, 1) == "1" then
					...
					surface.SetDrawColor(COLOR_25500255)
				...
				local npc_disp_t = i18n.GetPhrase("unknow")
				local npc_disp_color = color_white
				if npc_disp == 1 then
					npc_disp_color = COLOR_25500255
					npc_disp_t = i18n.GetPhrase("hostile")
				elseif npc_disp == 2 then
					npc_disp_color = COLOR_2551500255
					npc_disp_t = i18n.GetPhrase("frightened")
				elseif npc_disp == 3 then
					npc_disp_color = COLOR_02550255
					npc_disp_t = i18n.GetPhrase("friendly")
				elseif npc_disp == 4 then
					npc_disp_color = COLOR_2551500255
					npc_disp_t = i18n.GetPhrase("neutral")
				end
				...
				if string.sub(npc_info, 3, 3) == "1" then
					...
					surface.SetDrawColor(COLOR_505050150)
					surface.DrawTexturedRect(pos.x + npc_spacing - 2, pos.y + 55 - 2, 26, 26)
					surface.SetDrawColor(COLOR_102178255255)
				...
				if string.sub(npc_info, 4, 4) == "1" then
					...
					surface.SetDrawColor(COLOR_505050150)
					surface.DrawTexturedRect(pos.x + npc_spacing - 2, pos.y + 55 - 2, 26, 26)
					surface.SetDrawColor(COLOR_200255153255)
					surface.DrawTexturedRect(pos.x + npc_spacing, pos.y + 55, 22, 22)
					npc_spacing = npc_spacing + 32
				end
				...
				if string.sub(npc_info, 5, 5) == "1" then
					...
					surface.SetDrawColor(COLOR_505050150)
					surface.DrawTexturedRect(pos.x + npc_spacing - 2, pos.y + 55 - 2, 26, 26)
					surface.SetDrawColor(COLOR_2552130255)
					surface.DrawTexturedRect(pos.x + npc_spacing, pos.y + 55, 22, 22)
					npc_spacing = npc_spacing + 32
				end
				...
				if string.sub(npc_info, 6, 6) == "1" then
					...
					surface.SetDrawColor(COLOR_221160221255)
					surface.DrawTexturedRect(pos.x, pos.y + 70, 30, 30)
					draw.SimpleText(string.sub(npc_info, 7, -1), "VJFont_Trebuchet24_SmallMedium", pos.x + 32, pos.y + 75, COLOR_221160221255, 0, 0)
				end
			end
			
			local ent_hp = ply:GetNW2Int("vj_hud_trhealth")
			local ent_hpm = ply:GetNW2Int("vj_hud_trmaxhealth")
			if not ent:IsWorld() && not ent:IsVehicle() && ent:Health() != 0 then
				...
				draw.RoundedBox(1,pos.x,pos.y+30,190,20,COLOR_025525550)
				surface.SetDrawColor(0,255,255,150)
				surface.DrawOutlinedRect(pos.x,pos.y+30,190,20)
				draw.RoundedBox(1,pos.x,pos.y+30,hp_box,20,COLOR_0255255150)
				draw.SimpleText(string.format("%.0f", lerp_trace_hp)..hp_numformat, "VJFont_Trebuchet24_Small",(pos.x+105)-hp_num,pos.y+31,color_white)
			end
		end
	end
end)
...
local AbranknerVorKedne = {
	-- Barz Abrankner --
	gmod_button={Anoon = "Button", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_255255255150},
	edit_sky={Anoon = "Sky Editor", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_255255255150},
	edit_sun={Anoon = "Sun Editor", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_255255255150},
	edit_fog={Anoon = "Fog Editor", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_255255255150},
	
	-- Aroghchoutyoun yev Bashbanelik --
	item_healthkit={Anoon = "Health Kit", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	item_healthvial={Anoon = "Health Vial", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	item_battery={Anoon = "Suit Battery", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	item_suitcharger={Anoon = "Suit Charger", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	item_healthcharger={Anoon = "Health Charger", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	item_suit={Anoon = "HEV Suit", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_02550150},
	
	-- Panpousht --
	item_ammo_ar2={Anoon = "AR2 Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_ar2_large={Anoon = "Large AR2 Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_pistol={Anoon = "Pistol Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_pistol_large={Anoon = "Large Pistol Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_box_buckshot={Anoon = "Shotgun Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_357={Anoon = ".357 Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_357_large={Anoon = "Large .357 Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_smg1={Anoon = "SMG Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_smg1_large={Anoon = "Large SMG Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_ar2_altfire={Anoon = "Combine Ball Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_crossbow={Anoon = "Crossbow Bolts Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_ammo_smg1_grenade={Anoon = "SMG Grenade Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	item_rpg_round={Anoon = "RPG Ammo", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 300, DariKouyn = COLOR_000150},
	
	-- Vdankavor --
	combine_mine={Anoon = "Mine!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	prop_combine_ball={Anoon = "Combine Ball!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	npc_rollermine={Anoon = "RollerMine!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	grenade_helicopter={Anoon = "Bomb!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	npc_grenade_frag={Anoon = "Grenade!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	obj_vj_grenade={Anoon = "Grenade!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	fas2_thrown_m67={Anoon = "Grenade!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	doom3_grenade={Anoon = "Grenade!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
	cw_grenade_thrown={Anoon = "Grenade!", Negar = "negar/negar", DariDesag = "VJFont_Trebuchet24_Medium", Heravorutyoun = 400, DariKouyn = COLOR_255001},
}

hook.Add("HUDPaint", "vj_hud_proximityscanner", function()
	local ply = LocalPlayer()
	if not ply:Alive() or hud_enabled == 0 or GetConVar("vj_hud_scanner"):GetInt() == 0 then return end
	...
	for _, ent in pairs(ents.FindInSphere(ply:GetPos(), 320)) do
		...
		if v then
			...
			if math.Round(ply:GetPos():Distance(ent:GetPos())) < v.Heravorutyoun then
				...
				draw.SimpleText(VJ_ConvertToRealUnit(ply:GetPos():Distance(ent:GetPos())), "HudHintTextLarge", pos.x + 30, pos.y + 25, color_white, 0, 0)
			end
		end
	end
end)